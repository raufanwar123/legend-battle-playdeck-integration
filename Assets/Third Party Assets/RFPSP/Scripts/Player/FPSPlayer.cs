...
public class FPSPlayer : MonoBehaviour
{
    ...
    public Image newHealthBarFillImage;

    public Sprite Onbtn, OffBtn;
    public Image AutoFireBtn, autoFireBtnParent;
    public Button targetAssistBtn;
    public Image targetAssistTick;


    public Button autoFireBtn;
    public Image autoFireTick;
    ...
    //player hit points
    public float hitPoints = 30.0f; //100.0f //junaid change it 100 to 500 
    public float maximumHitPoints = 60.0f; //200.0f //junaid change it 200 to 500 
    ...
    bool FailFlag;
    [HideInInspector]
    public bool CrosshairFlag = true;
    [HideInInspector]
    public int healthLvl;
    public GameObject playerDeadImage; //junaid added this to show counter when player reborn

    //private void Awake()
    //{
    //    if (GameConfiguration.GetAutoFirePref() == 1)
    //    {
    //        AutoFireBtn.sprite = Onbtn;
    //        autoFireBtnParent.color = Color.green;
    //        TurnAutoFire = true;
    //    }
    //    else
    //    {
    //        AutoFireBtn.sprite = OffBtn;
    //        autoFireBtnParent.color = Color.white;
    //        TurnAutoFire = false;
    //    }
    //}
    private void Awake()
    {
        ...
        // Set value for Auto-Fire
        if (GameConfiguration.GetAutoFirePref() == 1)
        {
            TurnAutoFire = true;
            autoFireBtn.GetComponent<Image>().color = Color.green;
            autoFireTick.gameObject.SetActive(true);
            targetAssistBtn.interactable = false;
        }
        else
        {
            TurnAutoFire = false;
            autoFireBtn.GetComponent<Image>().color = Color.red;
            autoFireTick.gameObject.SetActive(false);
        }

        // Set value for Target Assist
        if (GameConfiguration.GetTargetAssist() == 1)
        {
            turnTargetAssist = true;
            targetAssistBtn.GetComponent<Image>().color = Color.green;
            targetAssistTick.gameObject.SetActive(true);
            autoFireBtn.interactable = false;
        }
        else
        {
            turnTargetAssist = false;
            targetAssistBtn.GetComponent<Image>().color = Color.red;
            targetAssistTick.gameObject.SetActive(false);
        }
    }
    void Start()
    {
        if (GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_ITEM_UNLOCKED + 0) == 1)
        {
            healthLvl = GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_ITEM_UNLOCKED + 0 + GameConfiguration.KEY_ITEM_LVL_NUMBER);
            Debug.Log("healthh Lvl : " + healthLvl);
            if (healthLvl == 1)
            {
                hitPoints += 15;
                maximumHitPoints += 15;
    ...
    {
        TurnAutoFire = !TurnAutoFire;
        if (TurnAutoFire)
        {
            AutoFireBtn.sprite = Onbtn;
            autoFireBtnParent.color = Color.green;
            GameConfiguration.SetAutoFire(1);
        }
        else
        {
            AutoFireBtn.sprite = OffBtn;
            autoFireBtnParent.color = Color.white;
            GameConfiguration.SetAutoFire(0);
        }
    }

    public bool TurnAutoFire = false;
    bool turnTargetAssist;
    int selectedWeapon;
    void LoadingScreenActive()
    {
        if (GameStat.instance)
            GameStat.instance.rebornScreen.SetActive(true);
        selectedWeapon = PlayerPrefs.GetInt("Selected_Gun");
        StartCoroutine(PlayerWeaponsComponent.SelectWeapon(selectedWeapon));
        MouseLookComponent.enabled = true;
        cameraAnimatorComponent.SetBool("PlayerDeath", false);
        hitPoints = 30f;
        newHealthBarFillImage.fillAmount = 1f;
        if (MyNPCWaveManager.instance)
            MyNPCWaveManager.instance.PlayerInitialization();
        if (GameStat.instance)
            GameStat.instance.oponentKills.text = " " + ++GameStat.oponentsPoints;
        FPSWalkerComponent.inputX = 1;
        FPSWalkerComponent.inputY = 1;
        FPSWalkerComponent.cancelSprint = false;
        PlayerWeaponsComponent.deadDropped = false;
        Invoke("LoadingScreenDeactive", 2);
    }

    int grenadesKit;
    public WeaponBehavior grenadeWeaponBehavior;
    public Text grenadesKitText;

    public void PickupGrenade()
    {
      //  GameConfiguration.setGrenade(1);
        grenadesKit = PlayerPrefs.GetInt(GameConfiguration.KEY_GRENADE, 0);
        PlayerPrefs.SetInt(GameConfiguration.KEY_GRENADE, grenadesKit + 1);
        grenadeWeaponBehavior.ammo = grenadesKit+1;
        grenadesKitText.text = (grenadesKit+1).ToString();
    }

    void LoadingScreenDeactive()
    {
        if (GameStat.instance)
        {
            GameStat.instance.rebornScreen.SetActive(false);
            if (GameStat.instance.zoomImage.activeSelf)
    ...
    void Update()
    {
        ...
        if(PlayerPrefs.GetInt(GameConfiguration.IsTragetAssistOn, 1)==0)
        {
            crosshairUiObj.SetActive(false);
            hitmarkerUiObj.SetActive(false);
        }
        else
        {
            crosshairUiObj.SetActive(true);
            hitmarkerUiObj.SetActive(true);
        }
        ...
        if (FailFlag && hitPoints <= 1)
        {
            crosshairEnabled = false;
            cameraAnimatorComponent.SetBool("PlayerDeath", true);
            if (GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_MODE) == 0)
            {
                GameStat.instance.FailFun("health");
            }
            FailFlag = false;
        }
        ...
        && !FPSWalkerComponent.hideWeapon)
        {
            if (!zoomStartState)
            {
                ...
                {//if button is tapped, toggle zoom state
                    ...
                    {
                        zoomed = true;
                        if (/*PlayerWeaponsComponent.currentWeapon == 2 ||*/PlayerWeaponsComponent.currentWeapon == 5 )
                        {
                            GameStat.instance.zoomImage.SetActive(true);
                        }
                    }
                    else
                    {
                        GameStat.instance.zoomImage.SetActive(false);  //   GameStat.instance.zoomImage.SetActive(true); 
                        zoomed = false;
                    }
                }
            }
        ...
        && !CameraControlComponent.thirdPersonActive)
        {
            ...
            if ((WeaponBehaviorComponent.meleeSwingDelay == 0 || WeaponBehaviorComponent.zoomIsBlock) && !WeaponBehaviorComponent.unarmed)
            {
                if (crosshairVisibleState)
                {
                    ...
                    crosshairUiImage.enabled = false;// Adeel
        ...
        if (crosshairEnabled)
        {
            ...
            //    crosshairUiImage.color = reticleColor;  Adeel
            }
            else
            {
                //make alpha of aiming reticle zero/transparent
    ...
    public void UpdateReticle(bool reticleType)
    {
        ...
        {
           // crosshairUiImage.sprite = aimingReticle; Adeel
            crosshairUiImage.color = reticleColor;
    ...
    public void HealPlayer(float healAmt, bool isHungryThirsty = false)
    {
        ...
        if (hitPoints + healAmt > maximumHitPoints)
        {
            ...
            // newHealthBarFillImage.fillAmount = hitPoints;
        }
        else
        {
            //Call Die function if player's hitpoints have been depleted
            ...
            print("Health FUll UI Update:" + hitPoints);
    ...
    public void ApplyDamage(float damage, Transform attacker = null, bool isMeleeAttack = false)
    {
        ...
        && WeaponBehaviorComponent.shootStartTime + WeaponBehaviorComponent.fireRate < Time.time)
        {
            ...
            //HUG DIrections Indicators

            //Damage HUD Directions
            //bl_DamageInfo info = new bl_DamageInfo(damage);

            //info.Sender = attacker.gameObject;

            //bl_DamageDelegate.OnDamageEvent(info);

            // attacker.gameObject.SetIndicator();
        ...
        //set health hud value to hitpoints remaining
        float health = hitPoints / 100;
        newHealthBarFillImage.fillAmount = health / (maximumHitPoints / 100);
        //print(health);
        HealthText.healthGui = Mathf.Round(hitPoints);
        HealthText2.healthGui = Mathf.Round(hitPoints);
        ...
        if (hitPoints < 1.0f)
        {
            if (GameStat.instance)
            {
                ...
                {
                    if(attacker)
                        GameStat.instance.killer = attacker.GetComponent<CustomAIController>().playerName;
                    GameStat.instance.killed = gameObject.tag + " 006";
    ...
    void Die()
    {
        if ((MyNPCWaveManager.instance.min > 0 || MyNPCWaveManager.instance.sec > 0) &&
            GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_MODE) == 1)   //junaid made this if else condition to reborn player  
        {
            Invoke("LoadingScreenActive", 3);
        }
        else
        {
    ...
    {
        if (!TurnAutoFire && turnTargetAssist == false)
        {
            // turn AutoFire on
            GameConfiguration.SetAutoFire(1);
            autoFireBtn.GetComponent<Image>().color = Color.green;
            ...
            // turn TargetAssist off
            GameConfiguration.SetTargetAssist(0);
            targetAssistBtn.GetComponent<Image>().color = Color.red;
            MouseLookComponent.isAutoAiming = false;
            MouseLookComponent.closestEnemy = null;
            targetAssistTick.gameObject.SetActive(false);
            turnTargetAssist = false;
        }
        else
        {
            ...
            targetAssistTick.gameObject.SetActive(false);
            turnTargetAssist = false;
            targetAssistBtn.interactable = true;
        }
    }

    public void TargetAssistToggle()
    {
        if (turnTargetAssist == false && TurnAutoFire == false)
        {
            // turn TargetAssist on
            GameConfiguration.SetTargetAssist(1);
            targetAssistBtn.GetComponent<Image>().color = Color.green;
            MouseLookComponent.isAutoAiming = true;
            MouseLookComponent.closestEnemy = null;
            targetAssistTick.gameObject.SetActive(true);
            autoFireBtn.interactable = false;
            turnTargetAssist = true;
    ...
    public void crosshairDisableOnly()
    {
        crosshairUiObj.SetActive(false);
       // Debug.Log("crosshair disabled only");
    }
}