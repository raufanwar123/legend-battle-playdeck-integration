...
public class InputControl : MonoBehaviour {
	...
	public bool injectHold;
	public void SetValueInjectHold(bool state)
	{
		if (CustomCharacterController.instance && CustomCharacterController.instance.totalMedicKitsAvailable > 0)
		{
			injectHold = state;
			FPSPlayerComponent.crosshairDisableOnly();
		}
	}
	...
	void Update () {
		...
		if(FPSPlayerComponent && !FPSPlayerComponent.restarting){
			...
			//{
			//	sprintHold = true;
			//	Debug.Log("Sprinting is Calling");
			//}
            ...
            //Debug.Log("Mouse X: " + lookX);
            //Debug.Log("Mouse Y: " + lookY);
			...
			if(ControlFreak2.CF2Input.GetAxisRaw("Xbox L Trigger") > 0.1f || ControlFreak2.CF2Input.GetButton("Zoom")){
				zoomHold = true;
				//Debug.Log("Zoom Pressed : " + zoomHold) ;
			}else{
				zoomHold = false;
				//Debug.Log("Zoom Pressed : " + zoomHold);
			}
            ...
            //		sprintHold = ControlFreak2.CF2Input.GetButton("Sprint"); //junaid comment it because sprint is on joystick
			...
			//selectNextPress = ControlFreak2.CF2Input.GetButtonDown("Select Next Weapon");                //Adeel
        ...
        if (moveInput.sqrMagnitude > 0.7)
        {
            ...
            FPSPlayerComponent.crosshairEnabled = false;
        }
        else
        {
            sprintHold = false;
            FPSPlayerComponent.crosshairEnabled = true;
        }
    }
