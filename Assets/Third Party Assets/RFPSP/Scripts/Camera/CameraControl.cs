...
//	public bool drawCamSphere;
	...
	//{
	//	RaycastHit hit;
	//	if (Physics.Raycast(transform.position, transform.forward, out hit, 800, layerMask))
	//	{
	//		if ((hit.collider.gameObject.tag == "Flesh" || hit.collider.gameObject.tag == "gun"))
	//		{
	//			if (hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.CharacterDamageComponent
	//				&& hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.factionNum != 1)
	//			{
	//				if (!hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.CharacterDamageComponent.isDead)
	//				{
	//					//FPSPlayerComponent.CrosshairFlag = false;
	//					if (GameConfiguration.GetAutoFirePref() == 1)
	//					{
	//						if (FPSPlayerComponent.turnAutoFire)
	//						{
	//							PlayerWeaponsComponent.CurrentWeaponBehaviorComponent.Fire();

	//						}
	//					}
	//				}
	//			}
	//			FPSPlayerComponent.crosshairUiObj.GetComponent<Image>().color = Color.red;
	//		}
	//		else
	//		{
	//			FPSPlayerComponent.crosshairUiObj.GetComponent<Image>().color = Color.green;
	//		}
	//	}
	...
	//	Debug.DrawRay(transform.position, transform.forward * 800f, Color.red);

		if (Physics.Raycast(transform.position, transform.forward, out hit, 800,layerMask))
        {
		//	print("CameraControl:: "+hit.collider.gameObject.name);
            if (hit.collider.gameObject.tag == "Flesh")
            {
                FPSPlayerComponent.CrosshairFlag = false;
                if (hit.collider.gameObject.GetComponent<LocationDamage>() && !hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.IsFriend)
                {
					if (!hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.CharacterDamageComponent.isDead)
                    {
						FPSPlayerComponent.crosshairUiImage.sprite = FPSPlayerComponent.aimingReticle2; //this makes the crosshair red
						if (FPSPlayerComponent.TurnAutoFire)
						{
							//if (PlayerWeaponsComponent.currentWeapon == 6 && !PlayerWeaponsComponent.CurrentWeaponBehaviorComponent.GetComponent<RocketLauncher>().isFire)
							//{
							//    PlayerWeaponsComponent.CurrentWeaponBehaviorComponent.GetComponent<RocketLauncher>().RocketLauncherFire();
							//}
							//else
							{
								PlayerWeaponsComponent.CurrentWeaponBehaviorComponent.Fire();
                ...
                //else if (hit.collider.gameObject.GetComponent<LocationDamage>() && hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.IsFriend)
                //{
                //    GameStat.instance.FireBtnDisable(false);
                //}
            ...
            else
            {
                ...
                FPSPlayerComponent.CrosshairFlag = true;
                FPSPlayerComponent.crosshairUiImage.sprite = FPSPlayerComponent.aimingReticle;
                if (!FPSPlayerComponent.TurnAutoFire /*&& PlayerPrefs.GetInt("Autoshoot") == 0*/)
                {
                    GameStat.instance.FireBtnDisable(true);
                }
            }
