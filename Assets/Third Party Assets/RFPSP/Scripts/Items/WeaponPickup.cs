...
public class WeaponPickup : MonoBehaviour {
	...
	public IEnumerator DestroyWeapon(float waitTime){
		...
		while (true){
			if(startTime + waitTime < Time.time){
				FreePooledObjects();
				CustomCharacterController.instance?.pickUpWeaponBtn.gameObject.SetActive(false);
   ...
   // { //junaid will added check here if multiplayer moede then muliplayer script run other wise EnemyCounter will run
   //     if (coll.tag == "Player")
   //     {
   //       EnemyCounter.instance.isPickUpGun = true; //junaid EnemyCounter
   //         PlayerWeapons PlayerWeaponsComponent = Camera.main.transform.GetComponent<CameraControl>().weaponObj.GetComponent<PlayerWeapons>();
   //         WeaponBehavior WeaponBehaviorComponent = weaponObj.GetComponent<WeaponBehavior>();
   //         if (weaponNumber == 3)
   //         {
   //             EnemyCounter.instance.pdrPickup_Weapon.haveWeapon = true;  //junaid EnemyCounter
   //             EnemyCounter.instance.pdrPickup_Weapon.cycleSelect = true;//junaid EnemyCounter
   //             EnemyCounter.instance.pdrPickup_Weapon.bulletsLeft = 10; //junaid EnemyCounter
   //             GameStat.instance.zoomImage.SetActive(false); //GameStat
			//	//Debug.Log("Zoom Image Band karo yar if me");
			//}
   ////         if (weaponNumber == 5)
   ////         {
   ////            EnemyCounter.instance.scriflePickUp_Weapon.haveWeapon = true; //junaid EnemyCounter
   ////             EnemyCounter.instance.scriflePickUp_Weapon.cycleSelect = true; //junaid EnemyCounter
   ////            EnemyCounter.instance.scriflePickUp_Weapon.bulletsLeft = 16;//junaid EnemyCounter
   ////             GameStat.instance.zoomImage.SetActive(false);//GameStat
			////	//Debug.Log("Zoom Image Band karo yar if me");
			////}
   //         PlayerWeaponsComponent.StartCoroutine(PlayerWeaponsComponent.SelectWeapon(WeaponBehaviorComponent.weaponNumber));
   ...
   // }
	...
	private void OnTriggerEnter(Collider other)
	{
		if (other.tag == "Player")
		{
			CustomCharacterController.instance.pickUpWeaponBtn.gameObject.SetActive(true);
			CustomCharacterController.instance.toDestroyPickup = gameObject;
			CustomCharacterController.instance.weaponToPickup = this;
		}
	}
	...
	private void OnTriggerExit(Collider other)
	{
		...
		{
			CustomCharacterController.instance.pickUpWeaponBtn.gameObject.SetActive(false);
			CustomCharacterController.instance.weaponToPickup = null;
