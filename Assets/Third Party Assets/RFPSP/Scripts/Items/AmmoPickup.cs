...
public class AmmoPickup : MonoBehaviour {
	...
	public IEnumerator DestroyWeapon(float waitTime)
	{
		...
		while (true)
		{
			if (startTime + waitTime < Time.time)
			{
				FreePooledObjects();
				Object.Destroy(gameObject);
				CustomCharacterController.instance.pickupHealthBtn.gameObject.SetActive(false); // Set pickup button to false when pickup grenade is destroyed
				yield break;
			}
			yield return null;
		}
	...
	private void OnTriggerEnter(Collider other)
	{
		if (other.tag == "Player")
		{
			CustomCharacterController.instance.pickupGrenadeBtn.gameObject.SetActive(true);
	...
	private void OnTriggerExit(Collider other)
	{
		if (other.tag == "Player")
		{
			CustomCharacterController.instance.pickupGrenadeBtn.gameObject.SetActive(false);
		}
	}
