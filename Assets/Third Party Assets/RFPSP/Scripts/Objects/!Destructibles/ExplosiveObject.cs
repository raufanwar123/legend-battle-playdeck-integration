			...
			/*&& 	Physics.Raycast(myTransform.position, ( hitColliders[i].transform.position - myTransform.position).normalized, out hit, radius*2f)*/){
				...
				if(explosionDamageAmt >= 1 /*&& hit.collider ==  hitCollider*/){
						...
						case 13://hit object is an NPC
                          //  Debug.Log("Hit An NPC");
							...
							if(hitCollider.gameObject.GetComponent<CharacterDamage>()){
								hitCollider.gameObject.GetComponent<CharacterDamage>().ApplyDamage(explosionDamageAmt, Vector3.zero, myTransform.position, myTransform, false, true,null,0,isMissile);
							}
							if(hitCollider.gameObject.GetComponent<LocationDamage>() && !hitCollider.gameObject.GetComponent<LocationDamage>().AIComponent.IsFriend)
							{
					...
					if (hitColliders[i].transform.GetComponent<Rigidbody>()){
                        ...
                        if (hitColliders[i].gameObject.layer == 13)
                        {
							if (GVSoundManager.Instance)
								GVSoundManager.Instance.PlaySound("hurt");
                        }
					}
				}
			}
