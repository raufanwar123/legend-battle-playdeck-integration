...
public class LocationDamage : MonoBehaviour {
			...
			if(isPlayer){//if attack is from player, pass damage info to main CharacterDamage.cs component
				...
				&& !isExplosion){
                    ...
                    if (headShot)
                    {
                        ...
                        PlayerPrefs.SetInt("Cash", PlayerPrefs.GetInt("Cash", 0) + 50);
                            if (EnemyCounter.instance)
                        {
                            EnemyCounter.instance.HeadSHotCOunter++;
                        }

                        Debug.Log("HeadShot:"+ EnemyCounter.instance.HeadSHotCOunter + "Total Coin:"+ PlayerPrefs.GetInt("Cash"));
                    }
                    if (GameStat.instance)
                    {
                        GameStat.instance.headShotImage.SetActive(true);
                        GameStat.instance.DeactiveObject();
                        //StartCoroutine(GameStat.instance.DisableHeadShot());
                    }

					//if(sloMoKillChance >= Random.value && isPlayer){
					//	AIComponent.PlayerWeaponsComponent.FPSPlayerComponent.StartCoroutine(AIComponent.PlayerWeaponsComponent.FPSPlayerComponent.ActivateBulletTime(1));
					//}
                    ...
                    {
                        UIController.instance.ShowHeadshotTxt();
                    }
				}
                ...
                if (GameStat.instance)
                {
                    ...
                    {
						if(attacker && attacker.GetComponent<CustomAIController>())
	...
	void OnCollisionEnter(Collision hit){
        if (AIComponent)
        {
            if (AIComponent.gameObject.GetComponent<CharacterDamage>().hitPoints > 0)
            {
