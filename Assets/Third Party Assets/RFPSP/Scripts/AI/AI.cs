...
public class AI : MonoBehaviour {
    ...
    [Tooltip("Check if NPC is from Player's Team in Multiplayer.")]
	...
	void Start(){
        ...
        //if (myTransform.gameObject.GetComponent<AI>().factionNum != 1) //comment by Imran
    ...
    public void ChangeTarget(Transform newTarget)   //Waseem
    {
        ...
        target = newTarget;
        lastVisibleTargetPosition = newTarget.position;
        StopCoroutine(AttackTarget());
        StartCoroutine(AttackTarget());
        speedAmt = runSpeed;
        TravelToPoint(newTarget.position);
    }
...
//			dirPerpGizmo.Normalize();
						...
						if(orderedMove && !followPlayer){
							...
							if(Vector3.Distance(playerObj.transform.position, myTransform.position) > pickNextDestDist){
								if(Vector3.Distance(playerObj.transform.position, myTransform.position) > pickNextDestDist * 2f){
									...
									if(lastRunTime + 2.0f < Time.time){
                                        speedAmt = runSpeed; //junaid walkSpeed; 		
                                    }
                                    }
								TravelToPoint(playerObj.transform.position);
							}else{
	...
	void CanSeeTarget(){
        ...
        //if (target != playerTransform)  //Waseem
        //    return;
		//target player
	...
	public void TravelToPoint( Vector3 position  ){
		if(attackFinished){
			agent.SetDestination(position);
			agent.isStopped = false;
            speedAmt = runSpeed;    //junaid added this
	...
	void SetSpeed( float speed  ){
		if (speed > walkSpeed && agent.hasPath){
			...
			if(speed > 0.0f && agent.hasPath){
                AnimatorComponent.SetInteger("AnimState", 2); //junaid 	AnimatorComponent.SetInteger("AnimState", 1);
