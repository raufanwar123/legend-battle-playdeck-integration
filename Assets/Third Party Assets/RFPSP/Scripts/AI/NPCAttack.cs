...
public class NPCAttack : MonoBehaviour {
    ...
    public bool isNewMuzzleFlashParticle;
    public GameObject newMuzzleFlashParticle;
	...
	void LateUpdate (){
		...
		if (muzzleFlash){
			if(Time.time - shootStartTime < fireRate * 0.33){ 
				if(mFlashState){
                    if (isNewMuzzleFlashParticle)
                    {
                        newMuzzleFlashParticle.SetActive(true);
                    }
                    else
				...
				if(!mFlashState){
                    ...
                    if (isNewMuzzleFlashParticle)
                    {
                        newMuzzleFlashParticle.SetActive(false);
                    }
				...
				switch(hit.collider.gameObject.layer){
					...
					case 13://hit object is an NPC
						...
						if(hit.collider.gameObject.GetComponent<LocationDamage>()){
                            ...
                            if (hit.collider.gameObject.GetComponentInParent<AI>().factionNum != 1)
                            {
                                if (hit.collider.gameObject.GetComponentInParent<AI>().agent != null)
                                {
                                    hit.collider.gameObject.GetComponentInParent<AI>().ChangeTarget(transform);
                                }
                            }
                            //if (hit.collider.gameObject.GetComponentInParent<CustomAIController>())
                            //{
                            //    GameStat.instance.killerName = gameObject.GetComponent<CustomAIController>().playerName;
                            //    GameStat.instance.killedName = hit.collider.gameObject.GetComponentInParent<CustomAIController>().playerName;
                            //    GameStat.instance.UpdateKillBar();
                            //}
                        }
						break;
					case 11://hit object is player
						if(hit.collider.gameObject.GetComponent<FPSPlayer>() && gameObject.GetComponent<AI>().factionNum !=1){
                            gameObject.SetIndicator();
                            hit.collider.gameObject.GetComponent<FPSPlayer>().ApplyDamage(damageAmt, myTransform, isMeleeAttack);
						}	
						if(hit.collider.gameObject.GetComponent<LeanColliderDamage>() && gameObject.GetComponent<AI>().factionNum != 1)
                        {
