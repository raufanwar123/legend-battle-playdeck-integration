...
public class PlayerWeapons : MonoBehaviour {
    ...
    int prevWepToInjectIndex;
    internal bool injectionState;


    void Awake()
	{
		firstWeapon=PlayerPrefs.GetInt("Selected_Gun");
	}


    IEnumerator ShouldEnableMediKit()
    {
        while(gameObject.activeSelf)
        {
            if(pullGrenadeState)
            {
                if(CustomCharacterController.instance.medicKitButton)
                    CustomCharacterController.instance.medicKitButton.interactable = false;  //Adeel
                yield return new WaitForSeconds(3f);
                if (CustomCharacterController.instance.medicKitButton)
                    CustomCharacterController.instance.medicKitButton.interactable = true;
    ...
    void Start()
    {
        ...
        StartCoroutine(ShouldEnableMediKit());
        ...
        if (firstWeapon == 17)
        {
            FPSPlayerComponent.TurnAutoFire = false;
        }
    ...
    void Update()
    {
        ...
        {//for enabling and disabling weapons when toggling cinema/alternate cameras and/or teleporting player
       ...
       // if(curr)
        ...
        if (injectionState)   //Adeel
        {
            StartCoroutine(SelectWeapon(prevWepToInjectIndex, false, false));
        ...
        && !FPSWalkerComponent.holdingObject)
        {
            ...
            if (InputComponent.selectGrenPress && !displayingGrenade) 
            {
                ...
                {//start counting grenades from zero if last grenade in list
            ...
            if (InputComponent.injectHold && CustomCharacterController.instance.totalMedicKitsAvailable > 0 && !CameraControlComponent.rotating ) //Salman 
            {
                CurrentWeaponBehaviorComponent.CancelWeaponPull();
                prevWepToInjectIndex = currentWeapon;
                StartCoroutine(SelectWeapon(12, true, false));
                //FPSPlayerComponent.HealPlayer(200f);
                InputComponent.injectHold = false;
            ...
            if (!sprintSwitching)
            {
                ...
                && !CurrentWeaponBehaviorComponent.dropWillDupe)
                {//if drop button is pressed and weapon isn't holstered (null weap 0 selected)
                ...
                if (Time.timeScale > 0.0f)
                {
                    ...
                    || InputComponent.xboxDpadLeftPress)
                    {//mouse wheel down or previous weapon button pressed
                        ...
                        {//not starting at zero
                            for (int i = currentWeapon; i > -1; i--)
                            {
                                ...
                                {//check that player has weapon and it is not currently selected weapon
                                    StartCoroutine(SelectWeapon(i));//run the SelectWeapon function with the next weapon index that was found
                                    break;
                                }
                                else if (i == 0)
                                {//reached zero, count backwards from end of list to find next weapon
                    ...
                    {//drop weapon button pressed and player has weapons in their inventory
                        ...
                        {//not starting at last weapon
                            for (int i = currentWeapon; i < weaponOrder.Length; i++)
                            {
                                ...
                                else if (i == weaponOrder.Length - 1)
                                {//reached end of list, count forwards from zero to find next weapon
    ...
    public void DropWeapon(int weapon)
    {
        ...
        {//only drop backup weapon if player dies
    ...
    public void UpdateTotalWeapons()
    {
        ...
        for (int i = 0; i < weaponOrder.Length; i++)
        {//iterate through weaponOrder array and count total weapons in player's inventory
            if (weaponOrder[i].GetComponent<WeaponBehavior>().haveWeapon && weaponOrder[i].GetComponent<WeaponBehavior>().addsToTotalWeaps)
            {
                //Debug.Log("Added  Weapon Name   :  " + weaponOrder[i].GetComponent<WeaponBehavior>().gameObject.name);
                totalWeapons++;//increment totalWeapons by one if player has this weapon	
            }
        }
        //Debug.Log("Total Weapon Added   :  " + totalWeapons);
    }
    ...
    /*
        Select Weapon By Same
    */
    public void SelectWeaponBySam(int index)
    {
       // print("selected weapon index : " + index);
    ...
    */
    ...
    {
        CurrentWeaponBehaviorComponent.haveWeapon = true;
        ...
        if (index != 0)
        {//if a weapon is selected, prevent unarmed/null weapon from being selected in selection cycle 
