...
public class WeaponBehavior : MonoBehaviour
{
    ...
    void Start()
    {
        if (GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_ITEM_UNLOCKED + 2) == 1)
        {
            ammoLvl = GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_ITEM_UNLOCKED + 2 + GameConfiguration.KEY_ITEM_LVL_NUMBER);
            UnityEngine.Debug.Log("Ammo Lvl : " + ammoLvl);
            if (ammoLvl == 1)
            {
                ammo += 15;
                maxAmmo += 15;
        ...
        if (GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_ITEM_UNLOCKED + 3) == 1)
        {
            fireRateLvl = GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_ITEM_UNLOCKED + 3 + GameConfiguration.KEY_ITEM_LVL_NUMBER);
            UnityEngine.Debug.Log("Fire Rate Lvl : " + fireRateLvl);
            if (fireRateLvl == 1)
            {
                fireRate -= 0.01f;
            }
            ...
            else if (fireRateLvl == 4)
            {
                fireRate -= 0.09f;
            }
        }
        if (GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_ITEM_UNLOCKED + 4) == 1)
        {
            reloadLvl = GameConfiguration.GetIntegerKeyValue(GameConfiguration.KEY_ITEM_UNLOCKED + 4 + GameConfiguration.KEY_ITEM_LVL_NUMBER);
            UnityEngine.Debug.Log("Reload Lvl : " + reloadLvl);
            if (reloadLvl == 1)
            {
                reloadTime -= 0.5f;
            }
        ...
        // Set crosshair size values
        restingSize = InputComponent.restingSize;
        maxSize = InputComponent.maxSize;
        speed = InputComponent.speed;
        ...
        if (!unarmed)
        {
            ...
            {//initialize muzzle flash color if not a melee weapon
                if (muzzleFlash)
                {
            ...
            if (semiAuto)
            {//make muzzle flash fade out slower when gun is semiAuto
                if (projectileCount < 2)
                {//make muzzle flash last slightly longer for shotguns
    ...
    public void InitializeWeapon()
    {
        ...
        if (!unarmed)
        {
            ...
            {//inventory of disposable, one-shot weapon has been depleted, don't show view model
        ...
        if (!unarmed && ((ammo > 0 || doReload) || meleeSwingDelay > 0.0f))
        {
            ...
            if (Time.timeSinceLevelLoad > 2 && PlayerWeaponsComponent.switching)
            {//don't ready weapon on level load, just when switching weapons
    ...
    void LateUpdate()
    {
        ...
        if (Time.timeScale > 0.0f && Time.deltaTime > 0.0f && Time.smoothDeltaTime > 0.0f)
    ...
    void Update()
    {
        if (firing || InputComponent.moveY > .85f)
        {
            currentSize = Mathf.Lerp(currentSize, maxSize, Time.deltaTime * speed); //Salman
        }
        else
        {
            currentSize = Mathf.Lerp(currentSize, restingSize, Time.deltaTime * speed);
        }
        FPSPlayerComponent.crosshairUiRect.sizeDelta = new Vector2(currentSize, currentSize); //Salman
        firing = false;
        ...
        {//allow pausing by setting timescale to 0
          ...
          //  UnityEngine.Debug.DrawRay(mainCamTransform.position, (weaponLookDirection + direction) * range, Color.red); Adeel
            ...
            if (!unarmed)
            {
                ...
                if (FPSPlayerComponent.hitPoints >= 1.0f)
                {
                    ...
                    if (doReload && !meleeActive)
                    {
                        ...
                        || FPSWalkerComponent.hideWeapon)
                        {
                            if (IronsightsComponent.reloading)
                            {
                                ...
                                {//if we are less than half way through reload before sprint interrupted, cancel reload
                    ...
                    {//dont shoot if player is climbing or weapon is hidden
                    ...
                    if (InputComponent.fireModePress || InputComponent.xboxDpadUpPress)
                    {
                           ...
                           && AnimatorComponent.GetCurrentAnimatorStateInfo(0).normalizedTime > 0.65f)
                        {
                            ...
                            if (fireModeSelectable && semiAuto)
                            {
                                ...
                                fireModeState = false;
                                if (projectileCount < 2)
                                {//make muzzle flash last slightly longer for semiAuto
                                    muzzleFlashReduction = 6.5f;
                                }
                                else
                                {
                                ...
                                if (burstAndAuto)
                                {//if all three fire modes are available, choose burst fire after semi auto
                                    if (!burstFire)
                                    {
                                        burstFire = true;
                                    }
                            ...
                            else if (fireModeSelectable && !semiAuto)
                            {
                                ...
                                {//if all three fire modes are available, choose auto fire after burst
                                    if (burstFire)
                                    {
                                        burstFire = false;
                                    }
            ...
            if (!unarmed && FPSPlayerComponent.hitPoints > 0)
            {
                   ...
                   && ((MouseLookComponent.inputY > -70 && !allowDownwardMelee) || allowDownwardMelee))
                {
                    ...
                    if (lastMeleeTime + meleeAttackTime < Time.time)
                    {
                        ...
                        {//play ready animation after offhand  melee attack, but fast forward to end to allow blending to reload anim if non-magazine reload
                ...
                && !waterFireState)
                {
                    ...
                    {
                        if (!burstFire)
                        {//fire weapon in auto mode
                            Fire();
                        }
                        else
                        {//fire weapon in burst mode
                ...
                if (burstState && burstFire)
                {
                    ...
                    {
                        if (burstShotsFired >= burstShots)
                        {//allow another burst to fire if fire button is released
                            burstState = false;
                            burstShotsFired = 0;
                        }
    ...
    public void Fire()
    {
        ...
        {
            if ((canShoot && !PlayerWeaponsComponent.switching) || doReleaseFire || meleeActive)
            {//don't allow shooting when reloading, sprinting, or switching
                ...
                if (((recoveryTime + recoveryTimeAmt < Time.time || CameraControlComponent.thirdPersonActive) || doReleaseFire || meleeActive) && (startTime + readyTimeAmt < Time.time))
                {
                    ...
                    else
                    {
                        ...
                        if (!audioDisabled)
                        {//reset firing timers because AudioSettings.dspTime tracks time differently than Time.time (audio engine init vs scene init)
                            lastFireTime = 0.0f;
                            nextFireTime = 0.0f;
                            audioDisabled = true;
                        }
                    ...
                    if (time > nextFireTime - schedulingTime)
                    {
                        ...
                        {//eject shell and perform muzzle flash if not a melee weapon
    ...
    IEnumerator FireOneShot()
    {
        ...
        else
        {
            ...
            {//determine which side to swing melee weapon, swing from left if this is a backstab
             //play camera view animation
                ...
                if (meleeSwingDelay == 0 && !meleeActive)
                {
                    ...
                    else
                    {
                        if (Physics.Raycast(shotOrigin, lookDirection, out hit, range, bulletMask))
                        {
                            ...
                            {//if raycast hit from player to target is very close, don't allow them to shoot around a corner in third person mode
        ...
        switch (hit.collider.gameObject.layer)
        {
            ...
            case 13://hit object is an NPC
                ...
                if (hit.collider.gameObject.GetComponent<LocationDamage>() && hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.enabled 
                    && !hit.collider.gameObject.GetComponent<LocationDamage>().AIComponent.IsFriend)
                {
//                    print("Hitting : " + hit.collider.gameObject.name);
        ...
        if (!isSecondCast)
        {
            ...
            if (hit.collider.gameObject.tag == "Flesh" || hit.collider.gameObject.tag == "Companion")
            {
                hitCount++;
                if (hitCount < 4)
                {//only draw one flesh impact effect if this is a shotgun for optimization
                    if (meleeSwingDelay == 0 && !meleeActive)
                    {
        ...
        while (barrelSmokeTime + barrelSmokeDuration > Time.time)
        {
               ...
               && !PlayerWeaponsComponent.offhandThrowActive)
            {
                if (!InputComponent.fireHold
                   ...
                   || ((shootStartTime + 0.1f < Time.time) && InputComponent.fireHold))
                {//set particle emission position
                    ...
                    {//do not emit particles when paused
    ...
    IEnumerator SpawnShell()
    {
        ...
        {//delay shell ejection for shotguns and bolt action rifles by shellEjectDelay amount
    ...
    public IEnumerator Reload()
    {
        ...
        {//prevent any reloading behavior at level start 
            ...
            {//dont allow reloading if gun is lowered while climbing, swimming, or holding object

                if (ammo > 0)
                {//if player has no ammo in their inventory for this weapon, do not proceed with reload

                    //cancel zooming when reloading
                    FPSPlayerComponent.zoomed = false;
                    if (GameStat.instance && GameStat.instance.zoomImage.activeSelf)
                    {
                    ...
                    {//If we are reloading weapon one bullet at a time (or bulletsToReload is less than the magazine amount) run code below
                     //determine if bulletsToReload var needs to be changed based on how many bullets need to be loaded						
                        ...
                        {//bullets needed are more or equal to bulletsToReload amount, so add bulletsToReload amount
                        ...
                        {//if bullets needed are less than bulletsToReload amount, just add the ammo that is needed
                            if (ammo >= bulletsNeeded)
                            {
                        ...
                        if (bulletsNeeded > 0)
                        {//if bullets still need to be reloaded and we are not loading a magazine
                        ...
                        {//if reloading last round, play normal reloading sound and also chambering effect
    ...
    void WeaponKick()
    {
        ...
        if (useRecoilIncrease)
        {
            ...
            else
            {
                ...
                {//less recoil when prone
                    spreadPerShot = 0.5f;
                }
            }

            if (useViewClimb && !MouseLookComponent.dzAiming)
            {//apply non-recoverable view climb to mouse input with sustained fire recoil
                if (viewClimbUp > 0.0f)
                {
        ...
        else
        {
            ...
            if (useViewClimb && !MouseLookComponent.dzAiming)
            {//apply non-recoverable view climb to mouse input without sustained fire recoil
                if (viewClimbUp > 0.0f)
                {
